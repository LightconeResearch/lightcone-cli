...
1. **Survey** — Fresh eyes. Read the constitution and the workdir's `AGENTS.md`. Check `git log`, glance at sub-fibers or notes the prior iteration left, look at what's actually in the workdir.
...
3. **Update** — Before exiting: commit your work; update `AGENTS.md`'s accumulators (Paper-vs-code disagreements, Open opportunities — whichever the project carries) if anything sharpened; sharpen the constitution body itself if a fact stable enough to belong in *Context* or *Desired State* landed.
4. **Exit** — `kill $PPID`.
...
**Discoverable updates.** Commits, files in the workdir, `AGENTS.md` accumulators — not progress notes scattered in the body. The next iteration finds what changed by inspecting the system.
