...
Its job is to render the validation surfaces, walk the user through the accumulated open questions, land the resolutions, and draft the final report — in one interactive arc. The Open opportunities list in AGENTS.md already carries un-acted-on opportunities from the latest COMPARE (those iterations logged them directly); REVIEW just reads them.
...
- `AGENTS.md` at the workdir root — paper identity, Rules, Paper-vs-code disagreements, Open opportunities (the durable surface, accumulated across iterations)
...
- AGENTS.md updates — **Paper-vs-code disagreements** entries reconciled with their resolutions (Open opportunities already there from COMPARE iterations)
...
8. **Re-running** — one paragraph: how to re-run from this workdir (`lc run --universe baseline`, the relevant `astra.yaml`, where AGENTS.md lives so future Claude Code sessions auto-load it on walk-up).
...
COMPARE iterations have been logging un-acted-on opportunities into AGENTS.md's *Open opportunities* list as they run, so the list is already populated. REVIEW's job here is reconciliation: cross-check that every opportunity in `comparison-report.yaml`'s `opportunities:` block that the user did NOT act on is present in AGENTS.md's list, and remove any that the user acted on at REVIEW (e.g. authorized one more IMPLEMENT round to close).

## Step 5: commit

Stage `REPRODUCTION-SUMMARY.md`, `open-questions.md` (with resolutions), the updated AGENTS.md, the final `astra.yaml`, the comparison artifacts, and any housekeeping changes. Commit with a message that names the verdict and the close-out:

```
review: <paper-short-name> verdict <verdict>, summary at REPRODUCTION-SUMMARY.md
```

This commit is the durable mark that the reproduction has reached close-out. Future walk-ups read AGENTS.md and `git log` to know where the reproduction stands; the close-out commit + REPRODUCTION-SUMMARY.md together stand in for the old constitution `outcome:` field.
...
- AGENTS.md's *Open opportunities* list reflects the un-acted-on opportunities from the latest COMPARE ⇒ reconciliation done
