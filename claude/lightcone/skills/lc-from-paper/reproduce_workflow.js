...
// tests/, scripts/, …) is resolved against the reproduction workdir. Agents inherit the
// LAUNCHING session's cwd — which is NOT guaranteed to be the workdir (e.g. the workflow
// launched from a parent project root). So we thread args.workdir through every agent:
//   - when it's '.' (the canonical "launched from the workdir" case) → CTX is empty,
//     behavior is identical to before;
//   - when it's an absolute path → CTX tells each agent to cd there first and resolve all
//     relative paths under it, and the spec/ledger constants are pinned absolute.
...
const CTX = WD === '.' ? '' :
  ...
  `requirements.txt, report.html, open-questions.md, CLAUDE.md) with \`${WD}/\`. ` +
  `Do not touch any astra.yaml or files outside \`${WD}\`. `
...
const base = agent
const A = (prompt, opts) => base(CTX + prompt, opts)
...
// preflight — the whole workflow's relative paths assume the workdir holds the ORIENT
// deliverable. Verify it does before authoring anything; a wrong-directory launch must
// die here, not silently build against a foreign astra.yaml in the session cwd.
const preflight = strip(await A(
  `Preflight check. Run exactly: test -f PLAN.md && test -d work/reference && echo OK || echo MISSING. ` +
  `Return ONLY the single word OK or MISSING.`,
  { label: 'preflight', phase: 'Architect' }
))
if (!preflight.includes('OK')) throw new Error(
  `reproduce-paper: PLAN.md / work/reference not found in the workdir (${WD}). ` +
  `Launch the workflow from the reproduction workdir, or pass args.workdir as an ABSOLUTE path to it.`
)
...
phase('Specify')
const subs = JSON.parse(strip(await A(
...
phase('Implement')
const outputs = JSON.parse(strip(await A(
...
const implemented = (await parallel(outputs.map(out => () => A(
...
const budgetInfo = JSON.parse(strip(await A(
...
const targets = JSON.parse(strip(await A(
...
await parallel(targets.map(t => () => A(
...
for (let round = 0; round <= MAX_ROUNDS; round++) {
  ...
  await parallel(verdict.failing.map(t => () => A(
