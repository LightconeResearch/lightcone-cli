...
- `reproduce_workflow.js` (ships with the `lc-from-paper` plugin, under `$CLAUDE_PLUGIN_ROOT/skills/lc-from-paper/`) — the autonomous middle: ARCHITECT → SPECIFY ∥ LITERATURE → IMPLEMENT → RUN → VERIFY → REVIEW. Phase contracts live in its `references/<phase>.md`.
