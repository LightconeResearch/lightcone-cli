...
- **Descriptions are orienting, not exhaustive.** ARCHITECT's `description:` blocks give a reader the shape of each analysis; the detailed prose (decision `rationale:`, finding `notes:`) lands on those entries in SPECIFY. A richer write-up — figures, citations, live numbers — is authored later as an external report that references analysis elements by their tree-path, outside `astra.yaml`.
