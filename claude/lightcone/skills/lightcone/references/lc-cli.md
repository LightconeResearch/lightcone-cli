# lc workflow reference (the `lc` CLI)

Running, debugging, and diagnosing the `lc` workflow: CLI commands, the Spec-Code Invariant, status interpretation, failure diagnosis, multiverse runs, scratch overrides for HPC, sub-analysis scaffolding, WRROC export. This is a reference bundled with the `/lightcone` skill; you don't activate it separately, you read the section you need. For `astra.yaml` spec syntax, see [astra.md](astra.md).
...
Populate the sub-analysis's `astra.yaml` with inputs, outputs, and decisions. Use `from:` references to wire inputs and decisions to the parent or siblings — see "Composition Mechanics" in [astra.md](astra.md) for the grammar.
