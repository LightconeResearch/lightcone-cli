...
**Follow the plan.** Read `.claude/build-plan-{{UNIVERSE}}.md` and work on the next unchecked item. The plan was designed with the right ordering — shared utilities before scripts that use them, upstream outputs before downstream ones. Trust it.
...
5. **If sub-agent says VERIFIED:** Clean up the build plan (`rm .claude/build-plan-{{UNIVERSE}}.md`), then output exactly: `<promise>BUILD_COMPLETE</promise>`
