"""Tests for the container runtime layer (podman-only).
...
class TestImageExistsLocally:
    ...
    def test_not_exists(self, mock_run: MagicMock) -> None:
        ...
        assert image_exists_locally("lc-foo", runtime="podman") is False
...
class TestBuildImage:
    ...
    def test_failure_raises(self, mock_run: MagicMock, project: Path) -> None:
        ...
        with pytest.raises(ContainerBuildError, match="podman build failed"):
...
class TestPullImage:
    ...
    def test_pull_success(self, mock_run: MagicMock) -> None:
        ...
        cmd = mock_run.call_args[0][0]
        assert cmd == ["podman", "pull", "python:3.12-slim"]

    @patch("lightcone.engine.container.subprocess.run")
    def test_pull_failure_raises(self, mock_run: MagicMock) -> None:
        mock_run.return_value = MagicMock(returncode=1, stdout="", stderr="boom")
        with pytest.raises(ContainerBuildError, match="podman pull"):
            pull_image("python:3.12-slim", runtime="podman")
...
class TestDetectRuntime:
    ...
    def test_podman_detected(self, mock_which: MagicMock) -> None:
    ...
    def test_podman_only(self) -> None:
        # podman is the sole container engine; other OCI CLIs return
        # later as new backends behind the image-layer abstractions.
...
class TestGetContainerStatus:
    def test_none(self, project: Path) -> None:
        s = get_container_status(None, project, "test", runtime="podman")
        assert s.type == "none"

    def test_prebuilt(self, project: Path) -> None:
        s = get_container_status("python:3.12", project, "test", runtime="podman")
    ...
    def test_containerfile_not_built(
        ...
        s = get_container_status("Containerfile", project, "test", runtime="podman")
        assert s.type == "build"
