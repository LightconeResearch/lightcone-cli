"""Plugin bundle discovery — finds the agent harness plugin shipped with lightcone-cli.
...
def get_plugin_source_dir() -> Path | None:
    """Find the lightcone agent harness plugin source directory.

    Looks for the plugin files in:

    1. Bundled location (installed package): ``lightcone/cli/plugin/lightcone/``
    2. Development location (repo): ``plugin/lightcone/`` relative to repo root
    """
    import lightcone.cli

    package_dir = Path(lightcone.cli.__file__).parent
    bundled_plugin = package_dir / "plugin" / "lightcone"
    ...
    dev_plugin = repo_root / "plugin" / "lightcone"
