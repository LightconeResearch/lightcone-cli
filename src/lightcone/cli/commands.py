...
def _write_artifacts(directory: Path, scratch_override: str | None) -> None:
    ...
    """
    # Plain (path, content) writers. One scaffold everywhere — the
    # Containerfile is agnostic to the execution environment. lightcone-cli
    # in requirements.txt brings the whole execution stack (snakemake, dask,
    # distributed, dask-gateway) so the same image can wrap recipes locally
    # or run as a Dask Gateway worker pod on a hub; anything pod-specific
    # (uid, mounts) is deployment configuration, not image content.
    writers = [
        ("Containerfile", _CONTAINERFILE),
        ("requirements.txt", _REQUIREMENTS + _lightcone_requirement()),
...
def _report_init(directory: Path, *, scratch_override: str | None) -> None:
    ...
    if directory != Path.cwd().resolve():
        console.print(
            f"  • Go to the directory [cyan]cd {directory}[/cyan]"
        )
    console.print(
