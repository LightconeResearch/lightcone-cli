...
A *pilot* is a long-lived SLURM allocation that hosts a persistent Dask
scheduler and pool of workers. After `lc pilot start`, every `lc run`
...
[`lc pilot`](../cli/pilot.md) reference documents the CLI surface.
...
`site_registry.SITE_DEFAULTS[site].pilot` and only appear in the YAML
when the user overrides them.

## Schema (multi-pool, mixed CPU + GPU)

When a single pilot needs to host both CPU and GPU workers, declare
multiple pools:
...
`lc pilot start` writes the rendered sbatch to
`<project>/results/.slurm/lc-pilot-<name>.sbatch`. A simplified single-pool
example:

```bash
#!/bin/bash
#SBATCH --job-name=lc-pilot-perlmutter
#SBATCH --nodes=4
#SBATCH --time=24:00:00
#SBATCH --qos=regular
#SBATCH --account=m1234
#SBATCH --output=results/.slurm/lc-pilot-perlmutter-%j.out

set -euo pipefail
module load python
source $HOME/.lightcone/envs/perlmutter/bin/activate

SCHED_FILE="$PSCRATCH/lightcone/pilots/perlmutter.json"
...
srun --nodes=4 --ntasks=4 --ntasks-per-node=1 \
     dask worker --scheduler-file "$SCHED_FILE" \
                 ...
                 --local-directory "$PSCRATCH/lightcone/pilots/scratch" &

wait "$SCHED_PID"
```

A multi-pool pilot emits one `srun` per pool, each with its own
`--constraint` and `--resources` tag.

## Worker env auto-bootstrap

The first `lc pilot start` for a site provisions
...
`worker_init` in the pilot YAML.

## QoS preflight

Before submitting, `lc pilot start` consults the cluster cache at
`~/.lightcone/cache/<site>.cluster.yaml` and validates the pilot's
...
```json
{
  ...
  "scheduler_file": "/pscratch/sd/u/u/lightcone/pilots/perlmutter.json"
}
```

The Dask scheduler address (`tcp://host:port`) is **not** cached — it's
read live from the scheduler-file each time `lc run` resolves the pilot,
so a pilot restart picks up a new address without intervention.

`lc pilot stop` cancels the SLURM job, removes the scheduler-file, and
deletes the state file. When SLURM hits walltime, the pilot dies cleanly
